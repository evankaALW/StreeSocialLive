...
// //       fetch(`${apiUrl}/getScreenDetails/${selectedTheater}`)
...
// //     fetch(`${apiUrl}/getTheatreDetails`)
// //       .then(response => response.json())
// //       .then(data => {
...
const renderDropdowns = (startDate, schedulerIndex) => {
  ...
  const renderSchedulers = () => {
    ...
    if (videos.length === 0 ) {
      ...
      if(!(theatreandscreen.data)){
        return (
          <h1>No Movies Allocated for the above Theater and Screen.</h1>
        );
      }
      else{
        return theatreandscreen.map((theatreAndScreenData, index) => {
  ...
  const renderDropdown = (schedulerIndex) => {
   ...
   }//if theatreandscreen is not empty
